...
namespace irods
{
	namespace indexing
	{
			...
			if (delay_err < 0) {
				THROW(delay_err,
				      boost::format("queue collection indexing failed for [%s] indexer [%s] type [%s]") %
				          _collection_name % _indexer % index_type);
			}
		                                                           ...
		                                                           std::vector<std::string> _resource_names)
		{
			...
			std::string query_str{boost::str(
				boost::format("SELECT RESC_NAME WHERE DATA_NAME = '%s' AND COLL_NAME = '%s'") % data_name % coll_name)};
			...
			THROW(SYS_INVALID_INPUT_PARAM,
			      boost::format("failed to find indexing resource for object [%s]") % _object_path);

		} // get_indexing_resource_name_for_object

		bool indexer::resource_is_indexable(const std::string _source_resource,
		                                    std::vector<std::string> _resource_names)
		{
					...
					for (const auto& row : metadata) {
						...
						if (_index_type == index_type) {
							...
							const auto policy_name =
								operation_and_index_types_to_policy_name(_operation_type, _index_type);
							schedule_policy_event_for_object(policy_name,
							                                 _object_path,
							                                 _user_name,
							                                 _source_resource,
		                                           ...
		                                           std::string& _unit)
		{
			...
			if (qobj.size() == 0) {
				THROW(CAT_NO_ROWS_FOUND,
				      boost::format("no results found for object [%s] with attribute [%s]") % _object_path %
				          _meta_attr_name);
			}
			...
			if (delay_err < 0) {
				THROW(delay_err,
				      boost::format("queue indexing event failed for object [%s] indexer [%s] type [%s]") %
				          _object_path % _indexer % _index_type);
			}

...
namespace irods
{
	namespace indexing
	{
			...
			: instance_name_{_instance_name}
		{
			try {
				...
				THROW(SYS_UNKNOWN_ERROR,
				      fmt::format(
						  "[{}:{}] in [file={}], [instance_name={}]", __func__, __LINE__, __FILE__, _instance_name));
			}
		                                                     ...
		                                                     const std::string& _index_type)
		{
			...
			THROW(SYS_INVALID_INPUT_PARAM, boost::format("operation [%s], index [%s]") % _operation_type % _index_type);
